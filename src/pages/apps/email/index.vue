...
async function fetchAllEmails() {
  try {
    const fetchEmails = await axios.get('https://prolific-tenderness-env.up.railway.app/api/gmail/read-all-mails')
...
// New ref to store single email data
const singleEmail = ref(null)

// Function to fetch a single email
async function fetchSingleEmail(emailId) {
  try {
    const response = await axios.get(`https://prolific-tenderness-env.up.railway.app/api/gmail/read-single-mails/${emailId}`)
...
const openEmail = async email => {
  await fetchSingleEmail(email.id)
  openedEmail.value = singleEmail.value//email
    ...
    <VMain>
      <VCard flat class="email-content-list h-100 d-flex flex-column">
        <div class="d-flex align-center">
          ...
          <!-- 👉 Search -->
          <VTextField v-model="q" density="default" class="email-search px-1 flex-grow-1" placeholder="Search mail">
            <template #prepend-inner>
              <VIcon color="disabled" size="22" icon="ri-search-line" />
            </template>
          </VTextField>
        </div>
        <VDivider />
        ...
        <div class="py-2 px-4 d-flex align-center">
          ...
          <VCheckbox :model-value="selectAllEmailCheckbox" :indeterminate="isSelectAllEmailCheckboxIndeterminate"
            class="me-1" @update:model-value="selectAllCheckboxUpdate" />
          <div class="w-100 d-flex align-center action-bar-actions gap-1" :style="{
      visibility:
              ...
              <VMenu activator="parent">
                <VList density="compact">
                  <template v-for="moveTo in emailMoveToFolderActions" :key="moveTo.title">
                    <VListItem :class="shallShowMoveToActionFor(moveTo.action) ? 'd-flex' : 'd-none'
      " class="align-center" href="#" @click="handleMoveMailsTo(moveTo.action)">
                      <template #prepend>
          ...
          <IconBtn class="me-1" @click="fetchEmails">
        ...
        <PerfectScrollbar tag="ul" :options="{ wheelPropagation: false }" class="email-list">
          <li v-for="email in getAllEmails" v-show="getAllEmails?.length" :key="email.id"
            class="email-item d-flex align-center pa-4 gap-2 cursor-pointer" :class="[{ 'email-read': email.id }]"
            ...
            <span class="text-body-2 truncate">{{ email?.headers?.subject }}</span>

            <VSpacer />

            <!-- 👉 Email meta -->
            <div class="email-meta align-center gap-2" :class="$vuetify.display.xs ? 'd-none' : ''">
              <VIcon v-for="label in email.labels" :key="label" icon="ri-circle-fill" size="10"
                :color="resolveLabelColor(label)" />

              <!-- <span class="text-sm text-disabled">
                {{ formatDateToMonthShort(email.date) }}
              </span> -->
            </div>
            <!-- 👉 Email actions -->
